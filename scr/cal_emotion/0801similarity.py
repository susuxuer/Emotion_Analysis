...
def calEmotion1(singleQ,singleA):
    ...
    for i in range(length):
        ques = singleQ[i]
        ans =singleA[i]
        pos_result, neg_result, score, tonescore, res = ce.single_review_sentiment_score(ques)
        pos_result1, neg_result1, score1, tonescore1, res1 = ce.single_review_sentiment_score(ans)
...
def calEmotion2(question, answer,len1):
    ...
    for i in range(len1):
        ...
        #计算情感
        toneQ,toneA = calEmotion1(singleQ,singleA)
        #sentToneA.append(toneA)
        allToneQ.append(toneQ)
        allToneA.append(toneA)      #allToneA保存的是一整个文本的所有语调
    print (len(allToneA))
    # for i in allToneA:
    #      print (i)
    return allToneQ,allToneA

def calSimilarity(Question, Answer):
    tfidfResult, lsiResult = K.similarity(Question, Answer)
    return tfidfResult, lsiResult
...
def saveSimilarity(code,short,tfidf0,lsi0):
    predictions = ['code','short','tfidf0','lsi0']
    df1 = pd.DataFrame({'code':code,'short':short,'tfidf0':tfidf0,'lsi0':lsi0},columns=predictions)
    df1.to_csv('D:/GFZQ/GFZQ/project/7_30_test/data/similarity/similarity0801.csv', index=False)
...
if __name__ == "__main__":
    ...
    code =[]
    for i in range(length):
        tfidfResult, lsiResult = calSimilarity(question[i], answer[i])
        print (companies[i][1])
        short.append(companies[i][1])
        code.append(companies[i][3])
        allTfidf.append(tfidfResult)
        allLsi.append(lsiResult)
    # print (allTfidf)
    # print (allLsi)
    #保存文件
    saveSimilarity(code, short, allTfidf, allLsi)
