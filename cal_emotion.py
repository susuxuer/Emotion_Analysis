...
def single_review_sentiment_score(weibo_sent):
    ...
    return pos_result,neg_result,result1,tone,res
...
if __name__ == '__main__':
    path = 'D:/GFZQ/GFZQ/xuesu2018/xuesu/克明面业2017年度业绩网上说明会.csv'
    wenben  = get_wenben(path)
    Q_all,A_all = get_QA(wenben)
    #length = 1
    #all_files =get_all_content()
    #Q_all, A_all = set_ourdict(all_files,length)
    pos = []
    res_su  = []
    for item in A_all: #Q_all[0]
        pos_result,neg_result,score,tonescore ,res=  single_review_sentiment_score(item)
        print (score,item)
        pos.append([pos_result,neg_result])
        res_su.append(res)
        #su.append([res,item])
        #neg.append(neg_result)
    result1 = 0
    result2 = 0
    for item1 ,item2 in pos:
        result1 +=item1
        result2 += item2
    ...
    score_column = pd.Series(res_su, name='score')
    answer_column = pd.Series(A_all, name='Answer')

    predictions = pd.concat([answer_column,score_column ], axis=1)
    df2 = pd.DataFrame({'Answer': A_all,'score': res_su})
    df2.to_csv("G:/project/sentimation_analysis/data/周大生corpus.csv",index=False)


    # with open("G:/project/sentimation_analysis/data/克明面业corpus.csv", "w", encoding="utf-8") as f:
    #     for score,item in su:
    #         f.write("{}\n".format(" ".join( [str(score),item] ) ) )

##用于比对
# if __name__ =="__main__":
#     path1 = "D:/GFZQ/GFZQ/data/test/周大生_computer.csv"
#     path2 = "D:/GFZQ/GFZQ/data/test/周大生_marked.csv"
#     auc,right,all_len = cal_AUC(path1,path2)
#     print ("right numbers:%d,all numbers: %d" %(right,all_len))
#     print('accuracy: {:.2%}'.format(auc))
